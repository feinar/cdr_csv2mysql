...
# 
# This script will insert data from asterisk cdr Master.csv file to mysql database after 
# checking  for а matching "calldate, src, duration" record in the database.
# 
# In my csv file all dates are 3 hours slow, so I use time correction 
...
for linia in `cat $1|sed s/'",,"'/'","","'/g` ; do
  ...
  # In csv time 3 hours slow:
  start_correction=$(date -d "$start 10800 seconds" +"%Y-%m-%d %H:%M:%S")
  ...
  echo $start $start_correction $clid $src $duration
  
    row_count=`mysql -h $DB_HOST -u $DB_USER -p$DB_PASSWORD  -D $DB_NAME -s -N -e "SELECT COUNT(1)
      FROM $DB_TABLE
       WHERE calldate='$start_correction'
       AND src='$src'
 ...
 if [ $row_count -eq 0 ] ; then         
        echo "Row ${row_count} does not exist, insert: $start $start_correction $clid $src $duration "; 
        echo "insert into $DB_TABLE values ('$start_correction','$clid','$src','$dst','$dcontext','$channel','$dstchannel','$lastapp','$lastdata','$duration','$billsec','$disposition','$amaflags','$accountcode','$userfield','$phone');" | mysql -h $DB_HOST -u $DB_USER -p$DB_PASSWORD  -D $DB_NAME
    else 
        echo 'Row exists, nothing to update';
 fi
